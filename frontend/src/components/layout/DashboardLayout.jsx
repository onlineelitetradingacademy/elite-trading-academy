...
export default function DashboardLayout() {
  ...
  useEffect(() => {
    ...
    checkUnread();
    const interval = setInterval(checkUnread, 5000); // every 5s
  ...
  return (
    <div className="min-h-screen bg-dark-900 flex">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-dark-800 border-r border-dark-500 transform transition-transform lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="flex flex-col h-full">
          <div className="p-5 border-b border-dark-500">
            <Link to="/" className="flex items-center gap-3">
              <div className="w-8 h-8 bg-gold rounded-lg flex items-center justify-center font-display text-dark-900 text-lg font-bold">
                E
              </div>
              <div>
                ...
                <div className="text-gold text-[9px] tracking-widest">
                  TRADING ACADEMY
                </div>
              </div>
            </Link>
          ...
          <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
            {navItems.map(({ label, href, icon: Icon, badge }) => (
              <NavLink
                ...
                onClick={() => {
                  if (label === 'Support') setUnreadCount(0);
                }}
                className={({ isActive }) =>
          ...
          <div className="p-4 border-t border-dark-500 space-y-2">
            {['admin', 'sub_admin'].includes(user?.role) && (
              <Link
                to="/admin"
                className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gold hover:bg-gold/10 transition-all"
              >
                ⚙️ Admin Panel
              </Link>
            )}
            <button
              onClick={logout}
              className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-muted hover:text-loss hover:bg-loss/10 w-full transition-all"
            >
