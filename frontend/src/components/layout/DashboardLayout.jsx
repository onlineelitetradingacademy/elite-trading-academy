...
import { FiHome, FiBook, FiBarChart2, FiUsers, FiUser, FiHelpCircle, FiMenu, FiX, FiLogOut } from 'react-icons/fi';
          ...
          <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
            {navItems.map(({ label, href, icon: Icon }) => (
              <NavLink key={href} to={href} end={href === '/dashboard'}
                className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all ${isActive ? 'bg-gold/10 text-gold border border-gold/20' : 'text-muted hover:text-white hover:bg-dark-600'}`}>
                <Icon size={16} /> {label}
              </NavLink>
            ))}
          </nav>
