...
export default function DashboardLayout() {
  ...
  const prevReplyCounts               = useRef({});     // ticketId -> replyCount last seen
  const isOnSupport                   = location.pathname.includes('/dashboard/support');
  ...
  const checkUnread = async () => {
    try {
      ...
      list.forEach(ticket => {
        ...
        const adminReplies = ticket.replies?.filter(r => r.isAdmin).length || 0;
        const prevCount    = prevReplyCounts.current[ticket._id] || 0;
        if (adminReplies > prevCount) {
          newCount += (adminReplies - prevCount);
          // Only update prev if user is currently viewing this ticket in support
          if (isOnSupport) {
            prevReplyCounts.current[ticket._id] = adminReplies;
          }
        }
      });

      // If user is on support page, mark all as read
      if (isOnSupport) {
        list.forEach(t => {
          prevReplyCounts.current[t._id] = t.replies?.filter(r => r.isAdmin).length || 0;
        });
        setUnreadCount(0);
      } else {
  ...
  // Initial load — establish baseline reply counts
  useEffect(() => {
    const init = async () => {
      try {
        const res  = await supportAPI.getMy();
  ...
  // Poll every 5s (user changed to 5000)
  useEffect(() => {
    const interval = setInterval(checkUnread, 5000);
    return () => clearInterval(interval);
  }, [isOnSupport]);

  // Clear badge instantly when user navigates to support
  useEffect(() => {
    if (isOnSupport) setUnreadCount(0);
  }, [isOnSupport]);
  ...
  return (
    <div className="min-h-screen bg-dark-900 flex">
          ...
          <div className="p-4 border-b border-dark-500">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gold/20 flex items-center justify-center overflow-hidden">
                {user?.avatar ? <img src={user.avatar} className="w-10 h-10 rounded-full object-cover" alt="" /> : <span className="text-gold font-bold">{user?.name?.[0]}</span>}
              </div>
              <div>
      ...
      <div className="flex-1 lg:ml-64">
        ...
        <div className="p-6 min-h-screen"><Outlet /></div>
      </div>
    </div>
  );
}
