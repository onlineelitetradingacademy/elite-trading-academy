...
export default function AdminSupport() {
  ...
  const fetchTickets = useCallback(
    async (refreshSelectedId = null, silent = false) => {
      try {
        ...
        if (silent && prevTickets.current.length > 0) {
          const newTickets = list.filter(
            (t) => !prevTickets.current.find((p) => p._id === t._id),
          );
          if (newTickets.length > 0) {
            toast(
              `🎫 ${newTickets.length} new ticket${newTickets.length > 1 ? 's' : ''} received!`,
              { icon: '🔔', id: 'new-ticket' },
            );
          }
          ...
          if (selected) {
            ...
            const prevSelected = prevTickets.current.find(
              (t) => t._id === selected._id,
            );
            if (
              freshSelected &&
              prevSelected &&
              freshSelected.replies?.length > prevSelected.replies?.length
            ) {
              const newUserReply = freshSelected.replies
                .slice(prevSelected.replies.length)
                .find((r) => !r.isAdmin);
              if (newUserReply)
                toast('💬 User replied to the selected ticket!', {
                  id: 'user-reply',
                });
            }
          }
          ...
          {statusList.map((s) => {
            ...
            return (
              ...
              >
                ...
                {count > 0 && (
                  <span
                    className={`text-xs font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center ${filter === s ? 'bg-gray-900 text-white' : badgeColor + ' text-white'}`}
                  >
                    {count}
                  </span>
                )}
              ...
              filtered.map((ticket) => (
                ...
                >
                  ...
                  <p className="text-gray-500 text-xs">
                    {ticket.user?.name || ticket.name || 'Guest'} •{' '}
                    {ticket.category}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-gray-500">
                    <span>
                      ...
                      <span className="text-white">
                        {selected.user?.name || selected.name || 'Guest'}
                      </span>
                    </span>
                    {(selected.user?.email || selected.email) && (
                      <span>📧 {selected.user?.email || selected.email}</span>
                    )}
                    {(selected.user?.phone || selected.phone) && (
                ...
                <div className="p-5 space-y-4 max-h-80 overflow-y-auto">
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 bg-gray-800 rounded-xl p-3">
                      <p className="text-white text-xs font-medium mb-1">
                        {selected.user?.name || selected.name || 'User'}
                      </p>
                  ...
                  {selected.replies?.length > 0 ? (
                    selected.replies.map((r, i) => (
                      ...
                      >
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${r.isAdmin ? 'bg-yellow-500 text-gray-900' : 'bg-blue-500/20 text-blue-400'}`}
                        >
                          {r.isAdmin
                            ? 'E'
                            : (selected.user?.name || 'U')[0]?.toUpperCase()}
                        </div>
                        <div
