...
import { FiPlus, FiEdit2, FiTrash2, FiUsers, FiClock, FiMail, FiPhone, FiX } from 'react-icons/fi';

const empty = { title:'', description:'', scheduledAt:'', platform:'zoom', joinLink:'', price:0, isFree:true, maxAttendees:100, status:'upcoming' };
const statusColors = { upcoming:'bg-blue-500/10 text-blue-400', live:'bg-green-500/10 text-green-400', completed:'bg-gray-700 text-gray-400', cancelled:'bg-red-500/10 text-red-400' };
...
export default function AdminWebinars() {
  ...
  const viewRegistrants = async (webinar) => {
    ...
    } catch { toast.error('Failed to load registrants'); setRegistrants(null); }
  ...
  return (
    <>
      ...
      <div className="space-y-6">
          ...
          <div className="fixed inset-0 bg-black/80 z-50 flex items-start justify-center p-4 pt-16 overflow-y-auto" onClick={() => setShowForm(false)}>
            <div onClick={e => e.stopPropagation()} className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-lg">
              ...
              <div className="space-y-4">
                ...
                <div><label className="text-xs text-gray-400 block mb-1">Join Link (Zoom/Meet URL)</label><input value={form.joinLink} onChange={e => f('joinLink', e.target.value)} placeholder="https://zoom.us/j/..." className="w-full bg-gray-800 border border-gray-600 focus:border-yellow-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none" /></div>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.isFree} onChange={e => { f('isFree', e.target.checked); if (e.target.checked) f('price', 0); }} className="accent-yellow-500" />
        ...
        {/* Registrants Modal */}
        {registrants && (
          <div className="fixed inset-0 bg-black/80 z-50 flex items-start justify-center p-4 pt-16 overflow-y-auto" onClick={() => setRegistrants(null)}>
            <div onClick={e => e.stopPropagation()} className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-2xl">
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-gray-500 text-sm">{registrants.webinar.title} • {registrants.users.length} registered</p>
                </div>
                <button onClick={() => setRegistrants(null)} className="w-8 h-8 rounded-lg bg-gray-800 flex items-center justify-center text-gray-400 hover:text-white"><FiX size={16} /></button>
              </div>
              ...
              {loadingReg ? (
                ...
                <div className="text-center py-10"><FiUsers size={32} className="text-gray-600 mx-auto mb-3" /><p className="text-gray-500">No registrants yet</p></div>
              ) : (
                <>
                  {/* Export info */}
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead><tr className="border-b border-gray-700">
                        {['#','Name','Email','Phone','Joined'].map(h => <th key={h} className="text-left px-3 py-2 text-xs font-semibold text-gray-400 uppercase">{h}</th>)}
                      </tr></thead>
                      <tbody>
                        {registrants.users.map((u, i) => (
                          <tr key={u._id} className="border-b border-gray-800 hover:bg-gray-800/50 transition-colors">
                            ...
                            <td className="px-3 py-3">
                              <a href={`mailto:${u.email}`} className="text-blue-400 hover:text-blue-300 flex items-center gap-1 text-xs"><FiMail size={11} /> {u.email}</a>
                            </td>
                            <td className="px-3 py-3">
                              {u.phone ? <a href={`tel:${u.phone}`} className="text-green-400 hover:text-green-300 flex items-center gap-1 text-xs"><FiPhone size={11} /> {u.phone}</a>
                                : <span className="text-gray-600 text-xs">—</span>}
                            </td>
                            <td className="px-3 py-3 text-gray-500 text-xs whitespace-nowrap">{new Date(u.createdAt).toLocaleDateString('en-IN')}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading ? [...Array(3)].map((_,i) => <div key={i} className="h-52 bg-gray-900 border border-gray-700 rounded-2xl animate-pulse" />)
          ...
          : webinars.map(w => (
            <div key={w._id} className="bg-gray-900 border border-gray-700 hover:border-yellow-500/20 rounded-2xl p-5 transition-all">
              ...
              <p className="text-gray-500 text-xs mb-2 flex items-center gap-1"><FiClock size={10} /> {w.scheduledAt ? new Date(w.scheduledAt).toLocaleString('en-IN', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' }) : 'TBD'} IST</p>
              <div className="flex items-center justify-between text-xs mb-3">
                <button onClick={() => viewRegistrants(w)}
                  ...
                  <FiUsers size={12} /> {w.registeredUsers?.length || 0} registered
              ...
              {w.joinLink && (
                <a href={w.joinLink} target="_blank" rel="noopener noreferrer"
                  className="text-xs text-blue-400 hover:text-blue-300 transition-colors truncate block">
                  🔗 {w.joinLink}
                </a>
              )}
              <p className="text-gray-600 text-xs mt-1 capitalize">{w.platform?.replace('_',' ')}</p>
            </div>
          ))}
        </div>
      </div>
