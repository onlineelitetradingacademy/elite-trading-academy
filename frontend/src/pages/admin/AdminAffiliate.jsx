...
export default function AdminAffiliate() {
  ...
  const fetchData = () => {
    setLoading(true);
    Promise.all([affiliateAPI.getAll(), affiliateAPI.getPayouts()])
      .then(([ar, pr]) => {
  ...
  const updateStatus = async (id, status) => {
    try {
      await affiliateAPI.update(id, { status });
  ...
  const processPayout = async (id, status) => {
    try {
      await affiliateAPI.processPayout(id, { status });
  ...
  const stats = [
    ...
    {
      label: 'Active',
      value: affiliates.filter((a) => a.status === 'approved').length,
      icon: FiTrendingUp,
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Manage affiliates and payout requests
          </p>
        </div>
                  ...
                  <thead>
                    <tr className="border-b border-gray-700 bg-gray-800">
                      {[
                        ...
                        <th
                          key={h}
                          className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase whitespace-nowrap"
                        >
                          {h}
                        </th>
                      ))}
                    ...
                    {loading ? (
                      ...
                      filtered.map((aff) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <code className="text-yellow-400 text-xs font-bold bg-yellow-500/10 px-2 py-1 rounded">
                              {aff.code}
                            </code>
                          </td>
                          ...
                          <td className="px-4 py-3 text-green-400 text-sm font-semibold">
                            ₹{aff.pendingEarnings?.toLocaleString() || 0}
                          </td>
                          <td className="px-4 py-3">
                            <span
                              className={`text-xs px-2.5 py-1 rounded-full border capitalize ${statusColors[aff.status] || 'bg-gray-700 text-gray-400'}`}
                            >
                              {aff.status}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex gap-2">
                              ...
                              {aff.status === 'approved' && (
                                <button
                                  onClick={() =>
                                    updateStatus(aff._id, 'suspended')
                                  }
                                  className="text-xs bg-gray-700 text-gray-400 px-3 py-1.5 rounded-lg hover:bg-gray-600 transition-all"
                                >
                                  Suspend
                                </button>
                              )}
                              {aff.status === 'suspended' && (
                                <button
                                  ...
                                  className="text-xs bg-green-500/10 text-green-400 px-3 py-1.5 rounded-lg hover:bg-green-500/20 transition-all"
                                >
                                  Reinstate
                                </button>
                              )}
                  ...
                  {loading ? (
                    ...
                    payouts.map((p) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`text-xs px-2 py-1 rounded-full ${p.status === 'completed' ? 'bg-green-500/10 text-green-400' : p.status === 'pending' ? 'bg-yellow-500/10 text-yellow-400' : 'bg-red-500/10 text-red-400'}`}
                          >
                            {p.status}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          {p.status === 'pending' && (
                            <div className="flex gap-2">
                              <button
                                onClick={() =>
                                  processPayout(p._id, 'completed')
                                }
                                className="text-xs bg-green-500 text-white px-3 py-1.5 rounded-lg hover:bg-green-400 transition-all font-medium"
                              >
                                Approve
                              </button>
                              <button
                                onClick={() => processPayout(p._id, 'rejected')}
                                className="text-xs bg-red-500/10 text-red-400 px-3 py-1.5 rounded-lg hover:bg-red-500/20 transition-all"
                              >
                                Reject
                              </button>
                            </div>
