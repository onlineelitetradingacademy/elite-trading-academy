...
import { FiPlus, FiMessageCircle, FiClock, FiCheckCircle, FiAlertCircle, FiRefreshCw } from 'react-icons/fi';
...
export default function MyTickets() {
  ...
  // Fetch all tickets and optionally refresh selected
  const fetchTickets = useCallback(async (selectId = null) => {
    try {
      ...
      const list = Array.isArray(res.data?.data)
        ...
        : [];
      setTickets(list);
      // Refresh selected ticket with latest data
      if (selectId) {
        const fresh = list.find(t => t._id === selectId);
        if (fresh) setSelected(fresh);
      } else if (selected) {
        const fresh = list.find(t => t._id === selected._id);
        if (fresh) setSelected(fresh);
      }
    } catch (err) {
      ...
      <div className="space-y-6">
              ...
              <form onSubmit={handleSubmit(createTicket)} className="space-y-4">
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => setShowForm(false)}
                    className="flex-1 border border-gray-600 text-gray-400 py-2.5 rounded-xl text-sm hover:text-white transition-colors">
                    Cancel
                  </button>
                  <button type="submit" disabled={submitting}
                    className="flex-1 bg-yellow-500 text-gray-900 font-bold py-2.5 rounded-xl text-sm disabled:opacity-60 hover:bg-yellow-400 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
          ...
          <div className="lg:col-span-2 space-y-3">
            {loading ? (
              [...Array(3)].map((_,i) => (
                <div key={i} className="h-24 bg-gray-900 border border-gray-700 rounded-2xl animate-pulse" />
              ))
            ) : tickets.length === 0 ? (
                ...
                <div className="p-5 space-y-4 max-h-80 overflow-y-auto">
                  ...
                  {selected.replies?.length > 0 ? (
                    ...
                    <div className="text-center py-4">
                      <p className="text-gray-500 text-xs">No replies yet. Our team will respond within 24 hours.</p>
                    </div>
                  )}
                </div>
