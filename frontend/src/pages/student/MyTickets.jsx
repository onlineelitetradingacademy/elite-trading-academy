...
export default function MyTickets() {
  ...
  const fetchTickets = useCallback(
    async (selectId = null, silent = false) => {
      try {
        ...
        if (prevTickets.current.length > 0) {
          ...
          list.forEach((ticket) => {
            ...
            const hasNewAdminReply = ticket.replies
              ?.slice(prevReplies)
              .some((r) => r.isAdmin);
            if (hasNewAdminReply) {
              newFlags[ticket._id] =
                (ticket.replies?.length || 0) - prevReplies;
              toast.success('💬 New reply from ELITE Support!', {
                id: `reply-${ticket._id}`,
              });
            }
          });
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              Get help from our team • auto-refreshes every 30s
            </p>
          </div>
          <div className="flex gap-2">
            {totalNewReplies > 0 && (
              <div className="flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 px-3 py-2 rounded-xl text-xs font-medium animate-pulse">
                <FiBell size={13} /> {totalNewReplies} new{' '}
                {totalNewReplies === 1 ? 'reply' : 'replies'}
              </div>
            )}
            <button
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full border capitalize ${statusColors[ticket.status] || 'bg-gray-700 text-gray-400'}`}
                      >
                ...
                <div className="p-5 space-y-4 max-h-80 overflow-y-auto">
                  ...
                  {selected.replies?.length > 0 ? (
                    selected.replies.map((r, i) => (
                      ...
                      >
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${r.isAdmin ? 'bg-yellow-500 text-gray-900' : 'bg-yellow-500/20 text-yellow-400'}`}
                        >
                    ...
                    <div className="text-center py-4">
                      <p className="text-gray-500 text-xs">
                        No replies yet. Our team responds within 24 hours.
                      </p>
                    </div>
                  )}
                </div>
