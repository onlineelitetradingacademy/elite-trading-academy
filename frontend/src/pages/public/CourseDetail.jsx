...
function EnrollCard({
  ...
  const checkCoupon = async () => {
    ...
    try {
      ...
      toast.success(
        `Coupon applied! You save ₹${res.data.coupon.discount.toLocaleString()} 🎉`,
      );
    } catch (err) {
        ...
        {isEnrolled ? (
          ...
          >
            {applying ? (
              ...
              <>
                <FiArrowRight size={16} /> Enroll Now — ₹
                {finalPrice.toLocaleString()}
              </>
            )}
          </motion.button>
          ...
          >
            {section.lessons?.map((lesson, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`flex items-center gap-3 px-5 py-3 border-t border-gray-700 text-sm ${i % 2 === 0 ? 'bg-gray-900' : 'bg-gray-850'}`}
                >
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${isCompleted ? 'bg-green-500/20 text-green-400' : isAccessible ? 'bg-yellow-500/10 text-yellow-400' : 'bg-gray-700 text-gray-500'}`}
                  >
                    {isCompleted ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <span
                      className={`${isAccessible ? 'text-white' : 'text-gray-500'} truncate block`}
                    >
                      {lesson.title}
                    </span>
                    {lesson.isPreview && (
...
export default function CourseDetail() {
    ...
    return (
      ...
      >
        <div className="text-center">
          <div className="w-12 h-12 bg-yellow-500 rounded-xl mx-auto mb-3 animate-pulse flex items-center justify-center font-display text-gray-900 font-bold text-xl">
            E
          </div>
  ...
  return (
    <>
      <Helmet>
        <title>
          {course.metaTitle || course.title} — ELITE Trading Academy
        </title>
        <meta
          name="description"
          content={course.metaDescription || course.subtitle}
        />
      </Helmet>
      ...
      <div
        ...
        className="pt-24 pb-8 border-b border-gray-800"
      >
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="flex-1 max-w-2xl">
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                <span
                  className={`text-xs font-semibold px-3 py-1 rounded-full border capitalize ${levelClass}`}
                >
                  {course.level}
                </span>
                <span className="text-xs font-semibold px-3 py-1 rounded-full bg-gray-700 text-gray-300 border border-gray-600">
                  {typeLabels[course.courseType] || course.courseType}
                </span>
                <span className="text-xs font-semibold px-3 py-1 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20 flex items-center gap-1">
                  <FiGlobe size={10} />{' '}
              ...
              <h1 className="text-white font-bold text-2xl md:text-3xl lg:text-4xl leading-tight mb-3">
                {course.title}
              </h1>
              {course.subtitle && (
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm mb-4">
                ...
                {totalLessons > 0 && (
                  <span className="flex items-center gap-1 text-gray-400">
                    <FiBookOpen size={13} /> {totalLessons} lessons
                  </span>
                )}
                {totalDuration > 0 && (
                  <span className="flex items-center gap-1 text-gray-400">
                    <FiClock size={13} /> {Math.floor(totalDuration / 3600)}h{' '}
              ...
              {activeTab === 'overview' && (
                ...
                >
                  ...
                  {course.whatYouLearn?.length > 0 && (
                    <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6">
                      <h2 className="text-white font-bold text-xl mb-5 flex items-center gap-2">
                        <FiCheckCircle className="text-yellow-400" /> What
                        You'll Learn
                      </h2>
                  ...
                  <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6">
                    ...
                    <div
                      className="prose-dark text-gray-300 text-sm leading-relaxed"
                  ...
                  {course.tags?.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {course.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-xs bg-gray-800 border border-gray-700 text-gray-400 px-3 py-1.5 rounded-full"
                        >
                          #{tag}
                        </span>
                      ))}
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <p className="text-gray-400 text-sm">
                      ...
                      lessons • {Math.floor(totalDuration / 3600)}h total
                    </p>
                    {!isEnrolled && (
                ...
                >
                  <div className="flex items-start gap-5 mb-5">
                    ...
                    <div>
                      ...
                      <p className="text-yellow-400 text-sm">
                        Founder & Head Mentor — ELITE Trading Academy
                      </p>
                  ...
                  {course.instructor.socialLinks && (
                    <div className="flex gap-3">
                      {Object.entries(course.instructor.socialLinks)
                        .filter(([, v]) => v)
                        .map(([platform, url]) => (
                          <a
                            key={platform}
