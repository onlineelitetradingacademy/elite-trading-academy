...
export default function Webinars() {
  ...
  const handleRegister = async (id) => {
    ...
    try {
      ...
      toast.success('Registered! Check your dashboard for the join link. 🎉');
      setWebinars((prev) =>
        prev.map((w) => (w._id === id ? { ...w, _registered: true } : w)),
      );
    } catch (err) {
  ...
  const recorded = webinars.filter(
    (w) => w.status === 'completed' && w.recordingUrl,
  );

  return (
    <>
      <Helmet>
        <title>Webinars — ELITE Trading Academy</title>
      </Helmet>
      <section
      ...
      >
        ...
        <div className="container-custom relative z-10 text-center max-w-2xl mx-auto">
          ...
          <p className="text-gray-400 text-lg">
            Join our live sessions — free and paid — on trading strategies,
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-gray-500 mb-6">
                Follow us on Instagram to get notified of upcoming webinars.
              </p>
            </div>
          ) : (
            <>
                    ...
                    {upcoming.map((w, i) => (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-3 mb-4 text-xs text-gray-500">
                          ...
                          {w.duration && (
                            <span className="flex items-center gap-1">
                              <FiPlay size={11} /> {w.duration} mins
                            </span>
                          )}
                          {w.maxAttendees && (
                            <span className="flex items-center gap-1">
                              <FiUsers size={11} />{' '}
                              {w.registeredUsers?.length || 0}/{w.maxAttendees}
                            </span>
                          )}
                        </div>
                        {w._registered ? (
                          <div className="flex items-center gap-2 text-green-400 text-sm font-medium">
                            <FiCheckCircle size={16} /> Registered! Check
                            dashboard for link.
                          </div>
                        ) : w.isFree ? (
                          <button
                          ...
                          <Link
                            to={`/checkout?item=${w._id}&type=Webinar`}
