      ...
      <div className="ticker-content gap-8">
        {doubled.map((item, i) => (
          <div key={i} className="flex items-center gap-2 flex-shrink-0 px-4">
            ...
            <span
              className={`text-xs font-medium ${item.up ? 'text-profit' : 'text-loss'}`}
            >
              {item.change}
            </span>
                    ...
                    >
                      {slide.title.split(' ').map((word, wi) =>
                        [
                          ...
                          <span key={wi} className="text-gradient">
                            {word}{' '}
                          </span>
                        ) : (
                          word + ' '
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16">
            <div className="flex-1 text-center lg:text-left">
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="inline-flex items-center gap-2 badge-gold mb-6 text-sm"
              >
              ...
              >
                Master Forex, Stocks, Crypto & Commodity trading with India's
                most comprehensive trading academy. Join{' '}
                <span className="text-gold font-semibold">
                  {settings?.stat_students || '5,000+'}
                </span>{' '}
                successful traders.
              </motion.p>
              <motion.div
              ...
              <div className="glass-card p-6 animate-float">
                ...
                <div className="h-24 relative mb-4">
                  <svg viewBox="0 0 300 80" className="w-full h-full">
                    <polyline
                      points="0,70 40,55 80,60 120,35 160,40 200,20 240,25 300,5"
                      fill="none"
                      stroke="#00C853"
                      strokeWidth="2.5"
                      strokeLinecap="round"
                    ...
                    />
                    <polyline
                      points="0,70 40,55 80,60 120,35 160,40 200,20 240,25 300,5 300,80 0,80"
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { l: 'Win Rate', v: '78%', c: 'text-profit' },
                    { l: 'Trades', v: '142', c: 'text-white' },
                    { l: 'Best Trade', v: '+₹42K', c: 'text-gold' },
                  ].map((s) => (
                    <div
                      key={s.l}
                      className="bg-dark-600 rounded-lg p-3 text-center"
                    >
...
function StatsSection({ settings }) {
  const stats = [
    ...
    },
    {
      ...
      value: settings?.stat_courses || '20+',
      icon: FiBookOpen,
      color: 'text-blue-400',
    },
    {
    ...
    },
    {
      ...
      value: settings?.stat_instagram || '12K+',
      icon: FiUsers,
      color: 'text-pink-400',
    },
  ];
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {stats.map((stat, i) => (
            ...
            >
              ...
              <div
                className={`font-display text-3xl md:text-4xl ${stat.color} mb-1`}
              >
  ...
  return (
    <motion.div whileHover={{ y: -4 }} className="course-card group">
      <div className="relative overflow-hidden">
        ...
        {course.isFree && (
          <span className="absolute top-3 left-3 badge-green">FREE</span>
        )}
        {course.isFeatured && (
          <span className="absolute top-3 right-3 badge-gold">⭐ Featured</span>
        )}
        <div className="absolute bottom-3 left-3 flex items-center gap-2">
          <span
            className={`text-xs font-medium capitalize ${levelColors[course.level]}`}
          >
            {course.level}
          </span>
          <span className="text-dark-400 text-xs">•</span>
      ...
      <div className="p-5">
        ...
        <h3 className="text-white font-semibold text-base mb-2 line-clamp-2 group-hover:text-gold transition-colors">
          {course.title}
        </h3>
        {course.instructor && (
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-xs text-muted">
            ({course.totalReviews || course.enrolledCount || 0})
          </span>
          <span className="text-xs text-muted ml-auto flex items-center gap-1">
            <FiUsers size={11} /> {course.enrolledCount || 0}
          </span>
        </div>
      ...
      <div className="container-custom">
        <div className="text-center mb-12">
          ...
          <h2 className="section-title">
            Our Expert <span className="text-gradient">Courses</span>
          </h2>
          <p className="section-subtitle mx-auto">
            From beginner fundamentals to advanced professional strategies — we
            have a course for every level of trader.
          </p>
        </div>
    ...
    <section className="py-20 bg-dark-800">
      <div className="container-custom">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-12">
          ...
          <Link
            to="/batches"
            className="btn-outline px-6 py-2.5 text-sm inline-flex items-center gap-2 whitespace-nowrap"
          >
          ...
          {batches.slice(0, 3).map((batch, i) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <span
                  className={`badge-${batch.type === 'online' ? 'blue' : 'gold'} text-xs capitalize px-3 py-1 rounded-full border ${batch.type === 'online' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-gold/10 text-gold border-gold/20'}`}
                >
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-white font-medium">
                    {batch.totalSeats - batch.enrolledSeats} left
                  </span>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-dark-500">
                <div>
                  ...
                  {batch.originalPrice && (
                    <span className="text-muted text-sm line-through ml-2">
                      ₹{batch.originalPrice?.toLocaleString()}
                    </span>
                  )}
                </div>
                <Link to={`/batches`} className="btn-gold text-sm py-2 px-4">
                  Book Seat
                </Link>
              </div>
            </motion.div>
            ...
            >
              <Link to={`/blog/${blog.slug}`}>
                <div className="h-48 bg-dark-700 overflow-hidden relative">
                  ...
                  <div className="absolute top-3 left-3 badge-gold text-xs capitalize">
...
function SegmentsSection() {
  const segments = [
    {
      icon: '📈',
      tag: 'Professionals Learning',
      title: 'For Loss-Makers & Beginners',
      desc: 'Are you losing money in markets or just starting out? Our structured programme takes you from zero to profitable trading with proven strategies.',
      features: [
        'Complete market education',
    ...
    },
    {
      icon: '🎓',
      tag: 'Student Module',
      title: 'Special Programme for Students',
      desc: 'Designed specifically for college and school students. Learn financial markets affordably on a monthly subscription model.',
      features: [
    ...
    },
    {
      ...
      desc: 'Our premium personal mentorship programme. Get individualised guidance on your actual trades for maximum growth.',
      features: [
    ...
    <section className="py-20 bg-dark-900">
      <div className="container-custom">
        <div className="text-center mb-16">
          ...
          <p className="section-subtitle mx-auto">
            We have tailored programmes for every type of learner — from
            complete beginners to experienced traders looking to level up.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {segments.map((s, i) => (
            ...
            >
              <div
                className={`absolute inset-0 bg-gradient-to-b ${s.color} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
              />
              <div className="relative z-10">
                {s.badge && (
                  <span className="absolute -top-2 -right-2 badge-gold text-xs">
                    {s.badge}
                  </span>
                )}
      ...
      <div className="container-custom relative z-10 text-center">
        ...
        >
          ...
          <p className="text-muted text-xl mb-10 max-w-2xl mx-auto">
            Join {settings?.stat_students || '5,000+'} traders who have already
            transformed their financial future with ELITE Trading Academy.
          </p>
          <div className="flex flex-wrap gap-4 justify-center">
            <Link
              to="/courses"
              className="btn-gold text-lg px-10 py-4 flex items-center gap-2"
            >
            ...
            >
              <FiPlay size={18} /> Try Free First
            </Link>
          </div>
          <p className="text-muted text-sm mt-6">
            ⚠️ Trading involves risk. Our courses are for educational purposes
            only.
          </p>
        </motion.div>
      </div>
  ...
  return (
    <>
      <Helmet>
        <title>
          {settings?.meta_title ||
            'ELITE Trading Academy — Where Traders Become Elite'}
        </title>
