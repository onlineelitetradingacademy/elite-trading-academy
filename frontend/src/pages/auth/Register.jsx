...
import { FiUser, FiMail, FiPhone, FiLock, FiEye, FiEyeOff, FiArrowRight, FiGift } from 'react-icons/fi';
...
export default function Register() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      navigate('/auth/verify-otp', { state: { userId: res.data.userId, email: data.email } });
    } catch (err) {
  ...
  return (
    <>
      <Helmet><title>Join ELITE Trading Academy — Create Account</title></Helmet>
      <div className="min-h-screen flex">
        ...
        <div className="hidden lg:flex flex-1 relative overflow-hidden bg-dark-800 items-center justify-center">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}
            ...
            <div className="font-display text-8xl text-white/5 absolute top-10 left-1/2 -translate-x-1/2 select-none">ELITE</div>
            <div className="glass-card p-8 mb-8 animate-float">
              <div className="text-5xl mb-4">🎓</div>
              <h3 className="text-white font-bold text-xl mb-2">Join ELITE Trading Academy</h3>
              <p className="text-muted text-sm mb-6">Get instant access to our free beginner workshop</p>
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <span className="text-sm text-muted">Free beginner workshop on signup</span>
            </div>
          </motion.div>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center p-8 pt-20 lg:pt-8 overflow-y-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
            <div className="text-center mb-8">
              ...
              <p className="text-muted">Start your journey to becoming an elite trader</p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Phone Number <span className="text-muted">(India)</span></label>
                <div className="relative">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
                    <FiPhone className="text-muted" size={14} />
                    <span className="text-muted text-xs">+91</span>
                  </div>
                  <input type="tel" placeholder="9876543210" className="input-field pl-16"
                    {...register('phone', { pattern: { value: /^[6-9]\d{9}$/, message: 'Invalid Indian phone number' } })} />
                </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Referral Code <span className="text-muted">(optional)</span></label>
                <input type="text" placeholder="Enter referral code (if any)" className="input-field uppercase"
                  {...register('referralCode')} />
              </div>

              <div className="flex items-start gap-3 pt-1">
                <input type="checkbox" id="terms" className="mt-1 accent-gold"
                  {...register('terms', { required: 'Please accept terms' })} />
                <label htmlFor="terms" className="text-sm text-muted">
                  I agree to the <Link to="/terms-of-service" className="text-gold hover:underline">Terms of Service</Link> and{' '}
                  <Link to="/privacy-policy" className="text-gold hover:underline">Privacy Policy</Link>
                </label>
              </div>
              {errors.terms && <p className="text-loss text-xs">{errors.terms.message}</p>}
            ...
            <p className="text-center text-muted text-sm mt-6">
              ...
              <Link to="/auth/login" className="text-gold hover:underline font-medium">Login here</Link>
            </p>
          </motion.div>
        </div>
      </div>
